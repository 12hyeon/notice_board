...
<body>
    <script>
        ...
        function cmp()
        {
            var cmp_id = document.getElementsByTagName("input")[0].value;
            <?php
            $conn = mysqli_connect('localhost', 'root', '011212', 'db');
            $sql = "SELECT * from member WHERE id='cmp_id'";
            echo "'cmp_id'";
            $result = mysqli_query($conn, $sql);
            if ($result) {
            echo "position[0].innerHTML = '중복된 아이디가 있습니다.'";
            } else {
            echo "position[0].innerHTML = '사용 가능한 아이디 입니다.'";
            } 
            ?>
        }
        ...
        <form action="sign_process.php" method="post">
            ...
            <div class="table_bgbox">
                <table>
                    <tr>
                        <td>아이디</td>
                        <td><input name="id" onchange ="check_id(this.value)" type="text" placeholder="4~12자리 영문+ 숫자"><button onclick="cmp()" value="중복확인"></button></td>
                    </tr>
